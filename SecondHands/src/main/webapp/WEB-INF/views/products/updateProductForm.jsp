...
<body>
   ...
   <div class="main-top">
      <form id="saveForm" action="${path}/products/update" method="post"
         ...
         <script>
    // 기존에 저장된 값을 받아오는 예시 (가정: 저장된 값이 savedCategory 변수에 있다고 가정)
    var savedCategory = "${product.board_cate}"; // 실제로는 서버에서 받아오거나 사용자 정보에서 가져와야 합니다.

    // 저장된 값이 있으면 해당 값으로 선택 상태를 설정
    var selectElement = document.getElementById("board_cate");
    if (selectElement) {
        for (var i = 0; i < selectElement.options.length; i++) {
            if (selectElement.options[i].value === savedCategory) {
         ...
         <label id="detail">가격:</label> <input type="number" id="board_Price"
               name="board_Price" step="1000" min="1000" max="999990000" required value="${product.board_Price}"
   ...
   <script>
    ...
    // 초기에 서울에 해당하는 중분류 추가
    categoryData['${product.loc_code}'].forEach(function (item) {
    ...
    $("#loc_code").on("change", function () {
        ...
        categoryData[key].forEach(function (item) {
         ...
         <script>
    // 기존에 저장된 값을 받아오는 예시 (가정: 저장된 값이 savedCategory 변수에 있다고 가정)
    var savedCategory = "${product.loc_code}"; // 실제로는 서버에서 받아오거나 사용자 정보에서 가져와야 합니다.

    // 저장된 값이 있으면 해당 값으로 선택 상태를 설정
    var selectElement = document.getElementById("loc_code");
    if (selectElement) {
        for (var i = 0; i < selectElement.options.length; i++) {
            if (selectElement.options[i].value === savedCategory) {
                selectElement.selectedIndex = i;
                break;
		         ...
		         <script>
    // 기존에 저장된 값을 받아오는 예시 (가정: 저장된 값이 savedCategory 변수에 있다고 가정)
    var savedCategory = "${product.detail_loc}"; // 실제로는 서버에서 받아오거나 사용자 정보에서 가져와야 합니다.

    // 저장된 값이 있으면 해당 값으로 선택 상태를 설정
    var selectElement = document.getElementById("detail_loc");
    if (selectElement) {
        for (var i = 0; i < selectElement.options.length; i++) {
            if (selectElement.options[i].value === savedCategory) {
                selectElement.selectedIndex = i;
